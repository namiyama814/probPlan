...
test("期限優先で3件の未完了タスクだけを取得できる", () => {
  ...
  assertEqual(recentTasks[0].task.name, "期限超過の未完了", "期限超過タスクを優先できません。");
  assertEqual(recentTasks[1].task.name, "期限が近い未完了", "期限が近いタスクを優先できません。");
  assertEqual(recentTasks[2].task.name, "期限なしの新しい未完了", "期限なしタスクの新しい順が正しくありません。");
  assert(
